...
//  Assignment3
//
//  Created by Krzysztof Dziewiszek on 7/9/16.
...
@IBDesignable class GridView: UIView {
         ...
         //   self.width = width
            ...
            let tmpArray = Array(count: height,
                                 repeatedValue: Array(count: width,
                                    repeatedValue: CellState.empty.description()))
    ...
    func t2(height: Int, width: Int) -> [[String]]
    {
        let tmpArray = Array(count: height,
                             repeatedValue: Array(count: width,
                                repeatedValue: CellState.empty.description()))
    ...
    override func touchesBegan(touches: Set<UITouch>, withEvent event: UIEvent?) {
        if let touch = touches.first {
            ...
            if grid[find.x][find.y] == "empty"
            {
                //currentState.toggle(.empty)
                grid[find.x][find.y] = "living"
                //print(grid[find.x][find.y])
                print(grid)
            } else
            {
                grid[find.x][find.y] = "empty"
            }
